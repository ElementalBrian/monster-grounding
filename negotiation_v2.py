...
def ollama(url, model, messages, stream=False, fmt=None):
    ...
    if fmt:
        payload["format"] = fmt
    return requests.post(url, json=payload, stream=stream, timeout=600)
...
def run_trial(mode, trial, fh, advisor_on=ADVISOR_ON):
    ...
    for turn, (spk, _) in zip(range(MAX_TURNS), itertools.cycle([(B, A), (A, B)])):
        ...
        # validate any price-shaped number against the authoritative appraisal
    ...
    final = judge_full("\n".join(transcript))
    truth = next((c["truth"] for c in claim_checks if c.get("truth")), None)
    deltas = [c["nearest_delta"] for c in claim_checks if c.get("nearest_delta") is not None]
    final.update({
        "turns": turn + 1,
        ...
        "quotes_matching_truth": sum(1 for c in claim_checks if c.get("matched")),
        "mean_abs_drift": round(statistics.mean(deltas), 1) if deltas else None,
        "final_drift": (abs(final["price"] - truth)
                        if truth and isinstance(final.get("price"), (int, float)) else None),
    })
...
def summarise(rs, name):
    ...
    matched = sum(r.get("quotes_matching_truth", 0) for r in rs)
    ...
    return (f"{name:9s}  deals {len(deals):>2}/{len(rs):<2}  price {p}  "
            f"|final-truth| {d}  quotes on-truth {matched}/{quoted}  "
            f"tool calls {tools}  leaked {leaks}")
